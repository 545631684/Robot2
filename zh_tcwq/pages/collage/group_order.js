    ...
    order: function() {
        ...
        app.util.request({
            ...
            success: function(t) {
                console.log("分页返回的列表数据", t);
                for (var a = 0; a < t.data.length; a++) t.data[a].status = o, t.data[a].xf_time = app.ormatDate(t.data[a].xf_time), 
                t.data[a].pay_time = app.ormatDate(t.data[a].pay_time), 15 <= t.data[a].receive_address.length && "" != t.data[a].receive_address && (t.data[a].receive_address = t.data[a].receive_address.slice(0, 15) + "...");
                var e = r.data.storelist;
                e = function(t) {
                    for (var a = [], e = 0; e < t.length; e++) -1 == a.indexOf(t[e]) && a.push(t[e]);
                    return a;
